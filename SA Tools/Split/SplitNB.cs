...
namespace SASplit.Split
{
	public static class SplitNB
	{
			...
			do
			{
				...
				switch(section_type)
				{
					...
					case AnimSections.MOTION:
						...
						uint mdataaddr = BitConverter.ToUInt32(file, section_addr);
						if(verbose)
						...
						mot.ModelParts = ByteConverter.ToUInt32(file, mdataaddr) >> ByteConverter.ToInt32(file, mdataaddr + 4);
						if(verbose)
							Console.WriteLine(", model parts: {0}", mot.ModelParts);
						uint tmpaddr = mdataaddr + 8; //Start of actual MDATA array
						for(int u = 0; u < mot.ModelParts; u++)
						{
							...
							{
								uint posoff = ByteConverter.ToUInt32(file, (uint)(tmpaddr + u * 16));
								data.PositionName = mot.Name + "_mkey_" + u.ToString() + "_pos_" + posoff.ToString("X8");
								int pos_count = ByteConverter.ToInt32(file, (uint)(tmpaddr + u * 16 + 8));
								//Console.Write(", position at: {0} ({1} entries)", posoff.ToString("X"), pos_count);
								for(int p = 0; p < pos_count; p++)
								{
									int index = ByteConverter.ToInt32(file, (uint)(posoff + p * 16));
									float pos_x = ByteConverter.ToSingle(file, (uint)(posoff + p * 16 + 4));
									float pos_y = ByteConverter.ToSingle(file, (uint)(posoff + p * 16 + 8));
									float pos_z = ByteConverter.ToSingle(file, (uint)(posoff + p * 16 + 12));
							...
							{
								uint rotoff = ByteConverter.ToUInt32(file, (uint)(tmpaddr + u * 16 + 4));
								data.RotationName = mot.Name + "_mkey_" + u.ToString() + "_rot_" + rotoff.ToString("X8");
								int rot_count = ByteConverter.ToInt32(file, (uint)(tmpaddr + u * 16 + 12));
								//Console.Write(", rotation at: {0} ({1} entries)", rotoff.ToString("X"), rot_count);
								for(int p = 0; p < rot_count; p++)
								{
									int index = ByteConverter.ToInt32(file, (uint)(rotoff + p * 16));
									int rot_x = ByteConverter.ToInt32(file, (uint)(rotoff + p * 16 + 4));
									int rot_y = ByteConverter.ToInt32(file, (uint)(rotoff + p * 16 + 8));
									int rot_z = ByteConverter.ToInt32(file, (uint)(rotoff + p * 16 + 12));
		...
		static NJS_OBJECT ProcessModel(byte[] file, bool verbose = false)
		{
			...
			while(curaddr < file.Length);
			return new NJS_OBJECT(file, (uint)file.Length - 52, 0, ModelFormat.Basic, null);
		}
