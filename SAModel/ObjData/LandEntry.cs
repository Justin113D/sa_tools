...
namespace SonicRetro.SAModel.ObjData
{
	...
	public class LandEntry
	{
		...
		public ModelData.Attach Attach
		{
			...
			set
			{
				if (value == null) throw new NullReferenceException("Attach cant be null!");
		...
		public static LandEntry Read(byte[] source, uint address, uint imageBase, AttachFormat format, LandtableFormat ltblFormat, Dictionary<uint, string> labels, Dictionary<uint, ModelData.Attach> attaches)
		{
			...
			if (ltblFormat < LandtableFormat.SA2) address += 8; //sa1 has unused radius y and radius z values
			...
			{
				unknown = ByteConverter.ToUInt32(source, address + 8);
				flags = ((SA2SurfaceFlags)ByteConverter.ToUInt32(source, address + 12)).ToUniversal();
			}
			else
			{
				flags = ((SA1SurfaceFlags)ByteConverter.ToUInt32(source, address + 8)).ToUniversal();
			}

			return new LandEntry(model, flags, blockBit, unknown, bounds);
		}
		...
		{
			if (labels.ContainsKey(_model.Name)) return;
		...
		{
			if (!labels.ContainsKey(_model.Name)) throw new InvalidOperationException("Model has not been written!");

			ModelBounds.Write(writer);
			if (ltblFormat < LandtableFormat.SA2) writer.Write(new byte[8]); //sa1 has unused radius y and radius z values
